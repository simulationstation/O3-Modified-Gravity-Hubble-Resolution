...
Ancillary predictive checks are directionally consistent but mixed: Pantheon SN-only transfer gives a weak positive Bayes factor ($\log\mathrm{BF}\!\approx\!+0.24$), while the all-transfer variant is mildly negative ($\log\mathrm{BF}\!\approx\!-0.48$). In the O3 injection-logit dark-siren score, the baseline posterior gives $\Delta\mathrm{LPD}=3.67$, while an entropy-derived direct re-score gives $\Delta\mathrm{LPD}=2.26$; leave-one-out on GW200308\_173609 gives 2.19 and 1.20, respectively. The support remains same-sign but reduced in amplitude, motivating full re-inference with larger siren samples.
\end{abstract}
\maketitle
...
We also replaced the phenomenological propagation posterior with the entropy-reconstruction posterior from the submission-hardening run and re-scored the same O3 injection-logit dark-siren set. The total support remains positive but is reduced:
\begin{equation}
...
The new predictive checks sharpen this interpretation. Pantheon SN-only transfer is weakly positive while all-transfer remains mildly negative, consistent with directional compatibility but not decisive closure. Hero-event forensics show concentration in GW200308\_173609 and a small set of high-leverage events, but no single-event collapse. The entropy-anchored direct re-score remains same-sign ($\Delta\mathrm{LPD}=+2.26$) but below the baseline ($+3.67$), indicating that entropy-derived propagation captures part, but not all, of the phenomenological O3 amplitude. The nonlinear bridge scan still identifies late-time transition families that satisfy both requirements simultaneously: core gate consistency near $\alpha\sim0.6$ and material-relief scale near $\alpha\sim2$.
...
\item \path{scripts/run_forward_tests_entropy_bridge_alignment.py}
\end{itemize}
