...
def event_specs() -> Dict[str, EventSpec]:
    base = ROOT / "outputs" / "forward_tests" / "hero_waveform_consistency_scaffold_20260210" / "events"
    return {
...
def build_jobs(args: argparse.Namespace, out_root: Path) -> List[JobSpec]:
    waveforms = [w.strip() for w in args.waveforms.split(",") if w.strip()]
    seeds = [int(s.strip()) for s in args.seeds.split(",") if s.strip()]
    events = list(event_specs().values())
